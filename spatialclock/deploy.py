...
def get_predictions(adata,
    ...
    for ct in celltypes:
        ...
        if f'{clock_obj_name}_{ct}.pkl' in os.listdir(os.path.join(directory,'clocks')): # if there exist clock for cell type
        
            print(f"Found clock for {ct}")

            # set up data
            sub_adata = adata[adata.obs["celltype"]==ct,:].copy()
            
            # read in clock genes and imputation values
            impdf = pd.read_csv(os.path.join(directory,f"clocks/mean_impute_files/{clock_obj_name}_{ct}.csv"))
            clock_genes = impdf.columns.copy()
            
            # # load training data to get genes -- normalized and smoothed but not standardized
            # tr_adata = sc.read_h5ad(os.path.join(directory,f"../clocks/{clock_obj_name}_{ct}.h5ad"))
            # clock_genes = os.path.join(directory,f"../clocks/{clock_obj_name}_{ct}.h5ad")
            ...
            if len(missing_genes) > 0:
                ...
                if fill_missing == "mean": # basic imputation with mean
                    ...
                    # #print(np.sum(np.isnan(sub_adata.X)))
            ...
            with open(os.path.join(directory,f"clocks/{clock_obj_name}_{ct}.pkl"), 'rb') as handle:
